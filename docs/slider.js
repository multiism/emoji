...
const linearInterpolate = (start, end, fraction) =>
...
class Slider {
    ...
    requestAnimationFrame(
      function() {
        const x = linearInterpolate(
          firstCanvas.offsetLeft,
          lastCanvas.offsetLeft,
          index / (this.faceCount - 1)
        );
        const y = linearInterpolate(
          firstCanvas.offsetTop,
          lastCanvas.offsetTop,
          index / (this.faceCount - 1)
        );
  ...
  animateSliderFaceExpression(previousIndex, index) {
    ...
    for (let frame = 0; frame < frameCount; frame++) {
      const interpolatedIndex = linearInterpolate(
        previousIndex,
        index,
        frame / (frameCount - 1)
      );
  ...
  showExpressionForIndex(index) {
    ...
    this.selectedFaceDisplay.update({
      emoji: makeSmileExpression(selectedSmileyness)
    });
  }
  ...
  render() {
    ...
    for (let face = 0; face < faceCount; face++) {
      ...
      const display = new EmojiDisplay(makeSmileExpression(smileyness), {
        size,
    ...
    const selectedFaceDisplay = (this.selectedFaceDisplay = new EmojiDisplay(
      makeSmileExpression(selectedSmileyness),
      { size }
    ));
